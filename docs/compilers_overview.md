...
Starting in CUDA Toolkit (CTK) 13.3, NVCC and PTXAS expose an __Advanced Controls Interface__ through the `--apply-controls` option. This interface lets you pass an __Advanced Controls File__ (ACF) to the compiler and change the compiler optimization choices used for that compilation.
